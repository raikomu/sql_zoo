...
SELECT matchid, mdate, COUNT(gtime) FROM game JOIN goal ON id = matchid
  WHERE (team1 = 'POL' OR team2 = 'POL') GROUP BY matchid, mdate

# 12. For every match where 'GER' scored, show matchid, match date and the number of goals scored by 'GER'
SELECT matchid, mdate, COUNT(gtime) FROM game JOIN goal ON id = matchid WHERE teamid = 'GER'
  GROUP BY matchid, mdate

# 13. List every match with the goals scored by each team as shown. 
SELECT mdate, team1, SUM(CASE WHEN teamid = team1 THEN 1 ELSE 0 END) score1, team2,
  SUM(CASE WHEN teamid = team2 THEN 1 ELSE 0 END) score2
  FROM game LEFT JOIN goal ON id = matchid GROUP BY mdate, matchid, team1, team2
